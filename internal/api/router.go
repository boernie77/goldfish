...
func (s *Server) Router() http.Handler {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Get("/libraries/{id}/genres", s.listGenres)
		...
		r.Get("/libraries/{id}/albums", s.listAlbums)
		r.Get("/albums/{id}", s.getAlbum)
		r.Get("/poster/album/{id}", s.getAlbumCover)
		r.Put("/albums/{id}/favorite", s.setAlbumFavorite)
		r.Put("/albums/{id}/metadata", requireAdmin(s.updateMusicAlbumMetadata))
		...
		// Eigene Schauspieler-Sektion der aufgegliederten Trefferanzeige
		// (User-Wunsch 2026-09-20) — separat von /items, siehe searchPeople.
		...
		r.Post("/items/move-preview", requireAdmin(s.moveItemsPreview))
		...
		r.Get("/admin/auto-backup/settings", requireAdmin(s.getAutoBackupSettings))
		r.Put("/admin/auto-backup/settings", requireAdmin(s.putAutoBackupSettings))
		r.Get("/admin/auto-backup/list", requireAdmin(s.listAutoBackups))
		r.Get("/admin/auto-backup/download/{name}", requireAdmin(s.downloadAutoBackupFile))
		r.Delete("/admin/auto-backup/{name}", requireAdmin(s.deleteAutoBackupFile))
		...
		r.Post("/libraries/{id}/folders/delete-watched-except-last", requireAdmin(s.deleteWatchedExceptLast))
